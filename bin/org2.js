...
io.on('connection', (socket) => {
 ...
 // Wait for 10 seconds
    ...
    try {
      // Don't convert to ObjectId since messageIds are stored as strings
    /*  console.log('Debug Info:');
      console.log('MessageId:', messageId);
      console.log('SenderId:', senderId);
      console.log('ReceiverId:', receiverId); */
  /*
      // First, let's find the document to verify it exists
      const conversation = await db.get().collection(collection.MESSAGES_COLLECTION)
        .findOne({
          participants: [
            { userId: senderId },
            { userId: receiverId }
          ],
          "messages.messageId": messageId  // Use string messageId
        });
  
      console.log('Found conversation:', JSON.stringify(conversation, null, 2));
  
     if (!conversation) {
        // Try reverse order of participants
        const conversationReverse = await db.get().collection(collection.MESSAGES_COLLECTION)
          .findOne({
            participants: [
              { userId: receiverId },
              { userId: senderId }
        ...
        console.log('Found conversation (reverse):', JSON.stringify(conversationReverse, null, 2));
      }
        */
  
      // Now attempt the update with the correct structure
      ...
      if (result.modifiedCount > 0) {
        ...
        /*
        // Verify the update
        const updatedConversation = await db.get().collection(collection.MESSAGES_COLLECTION)
          .findOne({
            $or: [
              {
                participants: [
